...
func (h *HeaderfileWriter) WriteNodeConfig(w io.Writer, cfg *datapath.LocalNodeConfiguration) error {
	...
	if iface := option.Config.EncryptInterface; len(iface) != 0 {
		// When FIB lookup is not supported we need to pick an interface
		// so pick first interface in list. Then we pick an IPv4 address
		// to use by selecting link IPAddr.
